...
_PID_FILE = PROJECT_ROOT / "runtime" / "futures_orderbook.pid"
_STOP_FILE = PROJECT_ROOT / "runtime" / "futures_orderbook.stop"
_LOG_DIR = PROJECT_ROOT / "logs"
_LOG_FILE = _LOG_DIR / "futures-orderbook-worker.log"
...
def _acquire_singleton() -> None:
    if _PID_FILE.exists():
        try:
            ...
            print(f"[futures-orderbook] Already running (PID {existing_pid}). Exiting.", flush=True)
            sys.exit(0)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the futures order book collector on a fixed interval.")
    ...
    try:
        ...
        while True:
            if _stop_requested():
                ...
                line = f"[{stopped_at}] futures order book collector stopped by flag: {_STOP_FILE}"
                print(line, flush=True)
                _write_log(line)
                record_heartbeat(
                    component="futures_orderbook_collector",
                    status="stopped",
                    message="futures_orderbook_collector stopped by flag.",
            ...
            record_heartbeat(
                component="futures_orderbook_collector",
                status="running",
                message="futures_orderbook_collector loop started.",
            ...
            try:
                ...
                line = f"[{started_at}] run={run_count} step=futures_orderbook status=error error={exc}"
                print(line, flush=True)
                _write_log(line)
                record_heartbeat(
                    component="futures_orderbook_collector",
                    status="failed",
                    message=f"futures_orderbook_collector loop failed: {exc}",
                    payload={"run_count": run_count, "interval_seconds": args.interval},
                )
