...
_PID_FILE = PROJECT_ROOT / "runtime" / "futures_premium.pid"
_STOP_FILE = PROJECT_ROOT / "runtime" / "futures_premium.stop"
_LOG_DIR = PROJECT_ROOT / "logs"
_LOG_FILE = _LOG_DIR / "futures-premium-worker.log"
...
def _acquire_singleton() -> None:
    if _PID_FILE.exists():
        try:
            ...
            print(f"[futures-premium] Already running (PID {existing_pid}). Exiting.", flush=True)
            sys.exit(0)
...
from app.pipeline.futures_premium_job import run_futures_premium_job
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the futures premium collector on a fixed interval.")
    ...
    try:
        ...
        while True:
            if _stop_requested():
                ...
                line = f"[{stopped_at}] futures premium collector stopped by flag: {_STOP_FILE}"
                print(line, flush=True)
                _write_log(line)
                record_heartbeat(
                    component="futures_premium_collector",
                    status="stopped",
                    message="futures_premium_collector stopped by flag.",
            ...
            record_heartbeat(
                component="futures_premium_collector",
                status="running",
                message="futures_premium_collector loop started.",
                payload={"run_count": run_count, "interval_seconds": args.interval},
            )
            try:
                connection = get_connection()
                try:
                    result = run_futures_premium_job(connection)
                finally:
                    connection.close()
                line = (
                    f"[{started_at}] run={run_count} step=futures_premium "
                ...
                line = f"[{started_at}] run={run_count} step=futures_premium status=error error={exc}"
                print(line, flush=True)
                _write_log(line)
                record_heartbeat(
                    component="futures_premium_collector",
                    status="failed",
                    message=f"futures_premium_collector loop failed: {exc}",
                    payload={"run_count": run_count, "interval_seconds": args.interval},
                )
