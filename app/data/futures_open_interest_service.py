...
_state = CollectionState(FUTURES_OPEN_INTEREST_PAUSED_FILE)
...
def _oi_url() -> str:
    return f"{futures_rest_base_url()}/fapi/v1/openInterest"
...
def fetch_futures_open_interest(symbol: str) -> dict[str, Any]:
    ...
    row = fetch_with_retry(_oi_url(), params=params, timeout=timeout, retries=_RETRIES, backoff=_BACKOFF).json() or {}
...
def _compute_open_interest_value(connection: DBConnection, symbol: str, timestamp_ms: int, open_interest: float | None) -> float | None:
...
def _compute_oi_change(connection: DBConnection, symbol: str, timestamp_ms: int, open_interest: float | None) -> tuple[float | None, float | None]:
    if open_interest is None:
        return None, None
    row = connection.execute(
        """
...
def collect_futures_open_interest_metrics(connection: DBConnection, symbol_names: list[str] | None = None) -> dict[str, Any]:
