...
def _fetch_all_since(symbol: str, start_ms: int, interval: str = "1m") -> list:
    ...
    cursor_ms = start_ms
    while True:
        klines = fetch_futures_klines(symbol=symbol, interval=interval, limit=PAGE_SIZE, start_ms=cursor_ms)
        if not klines:
            break
...
def _fetch_one(symbol: str, timeframe: str, start_ms: Optional[int], limit: int) -> Dict[str, Any]:
    if start_ms is not None:
        ...
        klines = fetch_futures_klines(symbol=symbol, interval=timeframe, limit=limit)
        mode = "seed"
